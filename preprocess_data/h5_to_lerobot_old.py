...
def detect_pose_anomalies(
    pose_data: np.ndarray, position_threshold: float = 0.03, tracker_name: str = "tracker"
) -> dict:
    """
    ...
    """
    anomalies = {"zero_frames": [], "jump_frames": [], "has_anomalies": False}
...
def extract_episode_files(hdf5_info):
    ...
    for key in hdf5_info:
        ...
        if info["lines"] is not None:
            with open(os.path.join(os.path.dirname(__file__), info["lines"])) as f:
                lines = f.readlines()
...
class Converter:
    ...
    def run(self):
        ...
        T_records = {}
        for ep_idx, (ep_path, line, feat, task) in enumerate(
            tqdm(zip(files, lines, feats, tasks), total=len(files), desc="Processing episodes")
        ):
            with h5py.File(ep_path, "r") as h5:
                try:
                    ...
                    if "pose_unified" in feat:
                        ...
                        if ep_idx == 0:  # Log format info for first episode
                            print(
                                f"Using separate pose devices: {left_pose.shape} from {left_pose_key}, {right_pose.shape} from {right_pose_key}"
                            )
                    ...
                    if self.enable_anomaly_detection:
                        ...
                        if has_episode_anomalies:
                            ...
                            # Update statistics
                            self.anomaly_stats["total_zero_frames"] += len(left_anomalies["zero_frames"]) + len(
                                right_anomalies["zero_frames"]
                            )
                            self.anomaly_stats["total_jump_frames"] += len(left_anomalies["jump_frames"]) + len(
                                right_anomalies["jump_frames"]
                            )

                            # Track maximum jump distance
                            for _, distance in left_anomalies["jump_frames"] + right_anomalies["jump_frames"]:
                                self.anomaly_stats["max_jump_distance"] = max(
                                    self.anomaly_stats["max_jump_distance"], distance
                                )
                    ...
                    if "pose_unified" in feat:
                        all_ts = [
                            ...
                            grip_left.shape[0],
                            grip_right.shape[0],
                            h5[feat["img_left"]].shape[0],
                            h5[feat["img_right"]].shape[0],
                        ]
                    else:
                        all_ts = [
                            left_pose.shape[0],
                ...
                for episode_info in self.problematic_episodes:
                    ...
                    if left_anom["has_anomalies"]:
                        ...
                        if left_anom["zero_frames"]:
                            print(
                                f"    - Zero frames: {len(left_anom['zero_frames'])} (indices: {left_anom['zero_frames'][:10]}{'...' if len(left_anom['zero_frames']) > 10 else ''})"
                            )
                    ...
                    if right_anom["has_anomalies"]:
                        ...
                        if right_anom["zero_frames"]:
                            print(
                                f"    - Zero frames: {len(right_anom['zero_frames'])} (indices: {right_anom['zero_frames'][:10]}{'...' if len(right_anom['zero_frames']) > 10 else ''})"
                            )
